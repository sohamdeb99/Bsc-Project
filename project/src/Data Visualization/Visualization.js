...
export default function Visualization() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/get-predictions')
      .then(response => {
        ...
        const formattedAdditionalMetrics = {
        ...
        setProtocolCounts(protocol_counts);
        setAnomalyData(anomaly_data);
        setAdditionalMetrics(formattedAdditionalMetrics);
        setLoading(false);

        // Check if there are anomalies and open the modal
        if (anomaly_data.abnormal > 0) {
          setOpenModal(true);
        }
      })
      .catch(error => {
  ...
  const totalProtocolsDetected = Object.values(protocolCounts).reduce((acc, count) => acc + count, 0);

  // Calculate percentages for anomalies and normal entries
  const totalEntries = anomalyData.normal + anomalyData.abnormal;
...
// Example data for the duration line chart
const durationData = {
  labels: additionalMetrics.duration.map((_, index) => `Instance ${index + 1}`),
  datasets: [{
    label: 'Duration',
    data: additionalMetrics.duration,
    fill: false,
    backgroundColor: 'rgba(153, 102, 255)',
    borderColor: 'rgba(153, 102, 255)',
    tension: 0.1 // This makes the line a bit curved. Set to 0 for straight lines.
  }]
};
    ...
    <div className="visualization-container">
      ...
      >
        <Box sx={{
          ...
          background: 'linear-gradient(313deg, #181835, #326291)', // Apply linear gradient background color
          boxShadow: 24,
          borderRadius: 2,
          p: 4,
          maxWidth: 400,
      ...
      {loading ? <div>Loading...</div> : error ? <div>Error: {error}</div> : (
        <>
          ...
          <div>
            ...
            <div>
              <h3>Source Bytes</h3>
              <Bar data={generateChartDataForMetric('Source Bytes', additionalMetrics.src_bytes)} options={chartOptions} />
            </div>
            <div>
              <h3>Destination Bytes</h3>
              <Bar data={generateChartDataForMetric('Destination Bytes', additionalMetrics.dst_bytes)} options={chartOptions} />
            </div>
          </div>
        </>
      )}
